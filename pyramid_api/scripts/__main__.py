...
import pyramid_api
from pyramid_api import scripts
from pyramid_api.settings import default_settings
...
DEFAULT_CONFIG_FILE = '/etc/pyramid_api/pyramid_api.ini'
...
@click.option(
    '-c', '--conf-file',
    type=str,
    help='configuration FILE, defaults to PYRAMID_API_CONF, use - for defaults',
    default=os.getenv('PYRAMID_API_CONF', DEFAULT_CONFIG_FILE),
)
@click.option(
    '-v', '--version',
    is_flag=True,
    help='print pyramid_api version',
)
@click.pass_context
def cli(ctx, log_level, conf_file, version):
    ...
    if version:
        click.echo(pyramid_api.__version__)
        return
